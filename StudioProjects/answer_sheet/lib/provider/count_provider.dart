...
class SubmissionNotifier extends ChangeNotifier {
  ...
  UnmodifiableListView<Submission> get submissions =>
      UnmodifiableListView(_submissionList);

  void add(Submission submission) {
    var result = _submissionList.firstWhere(
        (element) => element.name == submission.name,
        orElse: () => Submission(
            name: "-1",
            emailAddress: "",
            file: File(
                fileName: "", filePath: "", fileSize: "0", localFileName: "")));
    if (result.name == '-1') {
  ...
  Future<List<Submission>?> GetAllSubmission() async {
    ...
    Response response =
        await get(Uri.parse(getAllSubmissions), headers: headers);
