...
class studentData extends StatelessWidget {
  ...
  const studentData({Key? key, required this.idValue})
      : super(
            key:
                key); //created a class of studentsData which will take idValue as input argument.
  @override
...
class StudentDetails extends StatefulWidget {
  //creating class of StudentDetails which will take id as input argument
  final String id;
...
class _StudentDetailsState extends State<StudentDetails> {
  ...
  Future<PlatformFile?> _pickFile() async {
    //it will allow you to take file  using _pickleFile and return future(means ek future ko return karega.)
    ...
    final result = await FilePicker.platform.pickFiles(
        allowMultiple:
            true); //if u just remove await and u Point on (result) it becomes future.
  ...
  @override
  Widget build(BuildContext context) {
    //context store the widget location in widget tree
    ...
    final submissionProvider =
        Provider.of<SubmissionNotifier>(context, listen: false);
    return Form(
      key: _formKey,
      child: Padding(
                ...
                children: [
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: MaterialButton(
                          onPressed: () async {
                            var localFilePath =
                                await _pickFile(); //this will take file path as a string.as it is variable under future function.so, type await.
                            // ignore: use_build_context_synchronously
                            //create an object localFile of class file
                            File localFile = File(
                                fileName: localFilePath?.name,
                                localFileName: localFilePath?.name,
                                filePath: localFilePath
                                    ?.path); //filename and localFilePath have same name for a file.
                            //create an object of class Submission
                            submission = Submission(
                                name: _nameController.text,
                                emailAddress: _emailController.text,
                                file:
                                    localFile); //we cant type await here,'await' applied to 'Submission', which is not a 'Future
                          },
                          color: Colors.green,
                          child: const Text(
                            'Upload file',
                  ...
                  ElevatedButton(
                      onPressed: () async {
                        ...
                        await submissionProvider.addSubmission(
                            submission, submission.file.filePath!);
                      },
