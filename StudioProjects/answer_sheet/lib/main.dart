//stateless widget means that cant be rebuild. that means we cant change there appexarance or data. it is mutable
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      //Here , material app is parent for title, theme and primary search
      ...
      home: Scaffold(
        appBar: AppBar(
          title: const Text("Answer Submission"),
        ),
...
class _MyPageState extends State<MyPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        //padding is actually make the symmetric distance b/w the edge of border and inset editor.
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
        child: SingleChildScrollView(
          // it create a box in which single widget can be scrolled
          child: Column(
            children: [
              const SizedBox(
                height: 80,
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const studentLoginRoutes()),
                    );
